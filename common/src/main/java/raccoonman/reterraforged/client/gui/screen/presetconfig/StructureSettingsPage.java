	...
	public void init() {
		...
		registries.lookupOrThrow(Registries.STRUCTURE_SET).listElements().filter((holder) -> {
			...
			if(placement instanceof RandomSpreadStructurePlacement) {
				structures.structureSets.computeIfAbsent(holder.key(), (k) -> {
					return new StructureSetEntry(set, true);
				});
			}
		});

//		structures.structureSets.forEach((key, entry) -> {
//			StructurePlacement placement = entry.structureSet.placement();
//
